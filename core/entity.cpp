            ...
            for (int o = 0; o < boundObjects.size() && !contactX && !contactYbottom && !contactYtop; o++)
            {
                ...
                if (notCollidable == false) {
                    ...
                    for (int dir = 0; dir < 4; dir++) {
                        ...
                        while (boundObject->contains(boundObject->mapFromScene(QPointF(
                                     collisionPoints[dir * 2].x() + playerX + projectedMoveX,
                                     collisionPoints[dir * 2].y() + playerY + projectedMoveY
                                                     ))) ||
                               boundObject->contains(boundObject->mapFromScene(QPointF(
...
bool Entity::objectIsNotInCollisionPoints(QGraphicsItem * boundObject, int dir, qreal projectedMoveX, qreal projectedMoveY) {
    ...
    bool isPoint1Colliding = boundObject->contains(boundObject->mapFromScene(point1));
    bool isPoint2Colliding = boundObject->contains(boundObject->mapFromScene(point2));

    bool res = ! isPoint1Colliding && ! isPoint2Colliding;
